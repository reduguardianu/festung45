...
public class Renderer implements GLEventListener
{
	...
	Point3D rozrzut = new Point3D((float)(Math.PI/4), 0,(float)(Math.PI/4));
	...
	public void display(GLDrawable gLDrawable)
	{
		...
		{
			ogien = new SystemCzasteczkowy(texOgien[1], 3, frequency, new Point3D(0.0f, 1.5f, 0.0f), frequency, tick, new Point3D(0, 1.0f, 0), rozrzut, 1f, true);
			dym = new SystemCzasteczkowy(texDym[1], 6, frequency * 3, new Point3D(0.0f, 0.0f, 5.0f), frequency, tick, new Point3D(0, 1.0f, 0), rozrzut, 1f, true);
			kurz = new SystemCzasteczkowy(texKurz[1], 4, frequency, new Point3D(0.0f, 0.0f, -5.0f), frequency, tick, new Point3D(0, 1.0f, 0), rozrzut, 1f, true);
		}
		...
		gl.glLoadIdentity();
		glu.gluLookAt(interf.camPosition.x, interf.camPosition.y, interf.camPosition.z,  interf.camTarget.x, interf.camTarget.y, interf.camTarget.z,  0, 1, 0);
		  
		mapa.Render(gl);
		  
		Tiger.przesun(interf.tankPosition.x, interf.tankPosition.y, interf.tankPosition.z);
		Tiger.obroc("korpus", interf.tankTurn[0].x, interf.tankTurn[0].y, interf.tankTurn[0].z);
		Tiger.obroc("wieza", interf.tankTurn[1].x, interf.tankTurn[1].y, interf.tankTurn[1].z);
		Tiger.obroc("lufa", interf.tankTurn[2].x, interf.tankTurn[2].y, interf.tankTurn[2].z);
		...
		czas = (double)(interf.tick - oldTick) / interf.frequency;
		oldTick = interf.tick;
		
		deszcz.przemiesc(czas, interf.camTarget);
		deszcz.Render(gl);
		if(ogien != null)
		{
			ogien.przemiesc(czas, 0.0f, 1.5f, 0.0f);
			ogien.Render(gl, kamera);
			dym.przemiesc(czas, 0.0f, 0.0f, 5.0f);
			dym.Render(gl, kamera);
			kurz.przemiesc(czas, 0.0f, 0.0f, -5.0f);
			kurz.Render(gl, kamera);
		}
	...
	*/
	public void init(GLDrawable gLDrawable)
	{
		...
		Gaj = new Obiekt3D(parser.sprawdzLiczbeCzesci("3d/Gaj.ASE"));
		   
		parser.czytaj(Tiger, "3d/Tiger2.ASE", gl, glu);
		parser.czytaj(T34, "3d/T34-2.ASE", gl, glu);
		parser.czytaj(Gaj, "3d/Gaj.ASE", gl, glu);
		...
		mapa = new MapaWysokosci(gl, glu, "trawa.png", "mapa2.bmp", 1200, 5.0f);
	}
